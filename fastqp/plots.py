    ...
    for p in range(max_pos):
        for q in range(max_qual):
            try:
                ...
                pass
    imax = ax.imshow(np.array(heat_map), cmap=viridis_cm,
...
def nucplot(positions, nucs, counts, filename, fig_kw):
    ...
    fig, axes = plt.subplots(nrows=1, subplot_kw={
...
def gcdist(counts, filename, fig_kw):
    m = int(sum([k * v for k, v in zip(counts.keys(),
                                       counts.values())]) / sum(counts.values()))
    variances = [(k - m)**2 for k in counts.keys()]
    variance = int(
        sum([k * v for k, v in zip(variances, counts.values())]) / sum(counts.values()))
...
def kmerplot(positions, counts, filename, top_kmers, fig_kw):
    ...
    for pos, count in tuple(counts.items()):
        for kmer in kmers:
            ...
            if pos_count > 0:
                kmer_percent[pos][kmer] = float(
                    count.get(kmer, 0)) / pos_count * 100
            else:
                kmer_percent[pos][kmer] = 0.
    for kmer in [x for x in kmers if x not in top_kmers]:
        axes.plot(positions, [kmer_percent[pos][kmer]
                              for pos in positions], color='0.4', linestyle='dotted')
    legend_handles = []
    for kmer in top_kmers:
        legend_handles.append(
            axes.plot(positions, [kmer_percent[pos][kmer] for pos in positions])[0])
    # Shink current axis by 20%
    box = axes.get_position()
...
def adaptermerplot(positions, counts, adapters, filename, fig_kw):
    all_kmers = [counts[k].keys() for k in sorted(counts.keys())]
    kmers = tuple(
        set(list(itertools.chain.from_iterable(all_kmers))).intersection(adapters))
    kmer_len = len(tuple(kmers)[0])
    kmer_sums = Counter(dict(zip(
        kmers, [sum([counts[pos].get(kmer, 0) for pos in positions]) for kmer in kmers])))
    top_kmers = [x[0] for x in kmer_sums.most_common(9)]
    cmap = mpl.cm.get_cmap(name='Set1')
    colors = [cmap(i) for i in np.linspace(0, 1, len(top_kmers))]
    mpl.rcParams['axes.prop_cycle'] = mpl.cycler(color=colors)
    ...
    for label in legend.get_texts():
        label.set_color('black')
    add_figure_to_archive(
        fig, filename, 'cycle_specific_potiential_adapter_kmers.png')
...
def mismatchplot(positions, counts, filename, fig_kw):
    ...
    for ref in ['C', 'G', 'A', 'T']:
        for alt in set(['C', 'G', 'A', 'T']) - set([ref]):
            axes.plot(positions, [counts[ref][pos][
                      alt] / sum(counts[ref][pos].values()) for pos in positions])
